   ...
   <div class="profile-details">
      <div class="profile-details__main">
         <div class="profile-details__user">
            ...
            <div class="profile-details__user-data">
               <base-text :type="'subheading'">{{ profile.name }}</base-text>
               <base-text>{{ profile.gender }}</base-text>
               <base-text>{{ profile.position }}</base-text>
            </div>
         </div>
         <div class="profile-details__side">
            <div class="profile-details__actions" v-if="!public">
               <base-button @click="$emit('edit')">Edit Profile</base-button>
               <base-button @click="$emit('logout')" :variant="'ghost'">Logout</base-button>
            </div>
            <div class="profile__details__list">
               <div class="profile-details__list-item">
                  ...
                  <base-text>{{ profile.email }}</base-text>
               </div>
               <div class="profile-details__list-item">
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960">
                     <path
                  ...
                  <base-text>{{ profile.location }}</base-text>
               </div>
               <div class="profile-details__list-item" v-if="profile.visibility.phone === 'public'">
                  ...
                  <base-text>{{ profile.phone }}</base-text>
               </div>
               <div class="profile-details__list-item" v-if="profile.visibility.birthDate === 'public'">
                  ...
                  <base-text>{{ formatBirthday(profile.birthDate) }}</base-text>
               </div>
               <div class="profile-details__list-item" v-if="profile.hiredDate">
                  ...
                  <base-text> joined {{ formatJoinedDate(profile.hiredDate) }}</base-text>
               </div>
            </div>
         </div>
      </div>
      <div class="profile-details__about">
         <div class="profile-details__about-details">
            <base-text :type="'subheading'">{{ profile.name }} </base-text>
            <base-text as="router-link" :to="`/articles/users/${profile.id}/list`"
         ...
         <div class="profile-details__about--bio">
            <div class="article-preview__content" v-if="profile.bio && profile.bio.trim()" ref="bioRef"
               v-html="rendered"></div>
            <div v-else>
               <base-text :variant="'muted'">This profile hasn't added a bio yet</base-text>
            </div>
         </div>
      </div>
   </div>
